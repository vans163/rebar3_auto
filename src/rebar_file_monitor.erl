...
-module(rebar_file_monitor).

-export([start/0, start/1, start/2, stop/0, stop/1, monitor_file/2,
         monitor_file/3, monitor_dir/2, monitor_dir/3, rmmonitor/1,
         rmmonitor/2]).
...
rmmonitor(Ref) ->
    rmmonitor(?SERVER, Ref).

rmmonitor(Server, Ref) ->
    ok = command(Server, {rmmonitor, Ref}).
...
cast(Msg, Monitors) ->
    sets:fold(fun (#monitor{pid = Pid, reference = Ref}, Msg1) ->
                      %%erlang:display({file_monitor, Ref, Msg}),
                      Pid ! {file_monitor, Ref, Msg1},
                      Msg1  % note that this is a fold, not a map
              end,
             Msg, Monitors).
